...
2. 實際執行驗證，不可只靠推論：
   ...
   - `npm run build`
   - 有測試就跑 `npm test`
   - 有畫面就實際開起來看（手機寬度 + 桌機寬度各看一次）
3. 檢查有沒有殘留執行期錯誤（console error / warning / 未處理的 promise rejection）。
4. 清掉這一步產生的測試產物：測試用商品、測試用零件、暫時帳號、假資料、殘留字元、暫存檔。
5. 確認沒有破壞既有驗收情境（第 45 節 Case 1–10 中與這步相關的）。
