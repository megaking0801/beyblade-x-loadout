...
主 session 的模型必須依工作適配度、失敗風險與額度消耗選擇，不是永遠用最高階模型。每個新里程碑
開始前，主代理先用下表選出「能可靠完成工作的最低成本模型」；若建議與目前模型不同，要先明說
建議與原因，再由使用者實際切換。專案規則與代理不能自行改變既有主 session 的模型。

| Codex 模型 | 額度消耗 | 適用工作 | 升級條件 |
|---|---|---|---|
| Luna | 最低 | 固定格式轉換、檔案盤點、明確的單點查詢／機械式修改、可重跑的批次 | 需求有歧義、修改跨兩層以上，或第一次結果不符合既有模式時升 Terra |
| Terra | 低 | 預設選擇：一般 UI／domain 功能、既有模式內的測試與修正、有限範圍重構 | 需要跨層設計判斷、根因不明，或兩次嘗試仍未通過驗證時升 Sol |
| Sol | 中高 | 跨 UI／資料庫／domain 的重構、疑難除錯、來源或發布前判斷、需完整測試策略的變更 | 涉及不可逆 migration、資安／金鑰、全新架構或多工具研究，且失誤代價高時升 Astra |
| Astra | 最高 | 只有端到端且高風險的工作：架構定案、資料 migration／回復策略、影片資料管線與模型品質門檻、重大發布前審核 | 完成決策或高風險段落後立即降回 Terra／Sol；不得因檔案多就使用 |

額度是選型的一部分：Luna < Terra < Sol < Astra；先看目前用量／剩餘額度，再以任務範圍、
歧義、不可逆性和所需驗證強度決定。縮小任務範圍、減少無關 context、拆成可驗證小批次，
優先於無條件升級模型。`high` 以上 reasoning 僅用於真正需要規劃、分析或交叉驗證的段落；
`Ultra` 只在工作可獨立拆分、使用者明確同意委派，且不會讓代理共用同一工作樹時使用。

不同執行環境請用其原生控制項：

- **Claude Code**：用 `/model` 或啟動參數選定主模型。
- **Codex CLI**：在互動 session 輸入 `/model`；新 session 可用 `codex -m <model>`（或
  `codex --model <model>`）啟動。
- **Codex 桌面版／IDE**：在輸入框下方的模型與 reasoning 控制器選擇可用模型。Codex Cloud
  chat 目前不能變更預設模型。

Claude Code 的主 session 同樣由使用者依此規則選定。需要模型分工時，使用
`.claude/agents/*.md` 的 custom subagent，並在 frontmatter 固定 `model`，不可依賴 `inherit`。
