...
for (const row of rows) {
  ...
  const bladeId = bladeByZhName.get(row.site_blade_name)
...
console.log(`Raw Records 共 ${rows.length} 筆，比對成功 ${records.length} 筆，比對不到 ${unresolved.length} 筆`)
