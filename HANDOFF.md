...
寫給下一個接手的 session。最後更新：2026-09-18（第九段：視覺改版第二階段，4/11 頁）。

這份文件會被整份蓋掉重寫，不是逐段追加。要看歷史細節請用 `git log`。

---

## 現在在做什麼：把設計稿實作完（視覺改版第二階段）

最後一個 commit 是 `b449ca0`（首頁、我能組什麼、配裝比較、想買清單四頁照設計稿改版，
已推、已部署、`test:live` 綠）。工作區乾淨。

使用者最在意的一句是「整個網頁太醜」，設計稿（`design-draft/` 11 張）他看過也同意方向，
所以這一輪就是一頁一頁把稿實作完。**已完成 4 頁，剩 7 頁**：

| 頁 | 狀態 |
|---|---|
| 首頁 | ✅ 新增「賽場正在用什麼」、快速操作改兩欄卡片格 |
| 我能組什麼 | ✅ 序號 + 零件圖 + 迷你分數條 + 「差 N 件」三態徽章 |
| 配裝比較 | ✅ 「軸心 B → P」前後對照、每軸不同的勝出底色 |
| 想買清單 | ✅ 「最划算的一盒」、清單合計盒數 |
| 設定 | ⬜ 4 列資料來源分類表（官方／社群／未確認）、顯示模式改大卡、資料狀態卡裡補重載列（**複製一份，不要從首頁搬走**，`reload-latest` testid 在首頁有測試） |
| 商品 | ⬜ 「入門組」特色區、商品卡三種持有態（未持有／已有 ×N ＋再加一盒／剛加入） |
| 商品詳情 | ⬜ 頁內數量 stepper、頁內備註編輯、PageHeader 加返回 |
| 零件詳情 | ⬜ 3 欄統計條、規格 kv 表、「哪些盒子有」改精簡 chips、賽事名次徽章、加返回。⚠️ 設計稿這頁還寫著「重量 32g」，那是過期的，不要照抄 |
| 零件庫 | ⬜ 最大一頁。設計稿沒有分頁，是單一平面（搜尋 → 系列 chips → 類型 chips → 分組列）。現況是三分頁且「我的零件」連搜尋框都沒有 |
| 配裝器 | ⬜ 856 行最大。槽位列升級成 60px 卡、六軸改每軸不同色（現在恆用 `--accent` 單色）、高手評級獨立琥珀框、儲存＋分享改底部固定雙鈕列 |
| 3on3 | ⬜ 先做頂部庫存橫幅與「隊伍覆蓋」類型統計盒。設計稿還畫了**手動三槽組隊器**，那是新功能不只是改版，要先問使用者要不要做 |

完整的逐頁落差與硬約束清單在 `~/.claude/plans/eager-riding-lightning.md`。

### 這一輪順手修掉的全站問題

Tailwind preflight 有 `a { text-decoration: inherit }` 與 `ol,ul,menu { list-style: none }`，
所以「來源」連結原本長得跟普通文字一樣（違反規格第 22 節「看得到並查得回去」），
13 個說明清單也只是一堆縮排句子。已用 `a:not([class])` 與全域 `ul { list-style: disc }` 補回來
——元件型連結的 class 都掛在 `Link` 上，所以裸 `<a>` 選擇器不會波及它們。
**preflight 本身不要拔掉**：`box-sizing: border-box` 與 margin reset 是現有版面的隱含前提。
...
- **整套測試**（`npm test`、`npm run test:e2e`、`npm run shots`）只在一次大改完成後與上線前跑。
- 寫新功能時該功能自己的測試照 TDD 先紅後綠，但只跑那一個檔。

已寫進 `BEYBLADE_X_codex_prompt.md` 第 47.1／47.6 節與 `CLAUDE.md`。

**踩過的坑：不要邊派紅隊邊改同一批檔案。** 這次紅隊審查中途檔案被我自己改掉，
它花了額外篇幅在確認「到底審的是哪一版」。派出去之後就去改別的檔案，或等它回來。
...
- 最大的未完成品仍是**視覺**：11 張設計稿已實作 4 頁，剩 7 頁（見上面的表）。
